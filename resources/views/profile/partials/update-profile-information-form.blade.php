<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
                ...
                <div>
                    <p class="text-sm mt-2 text-zinc-200">
                        ...
                        <button form="send-verification"
                            class="underline text-sm text-zinc-400 hover:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        ...
        <div>
            <x-input-label for="favorite_team" :value="__('Favorite Team')" class="text-white" />
            <select id="favorite_team" name="favorite_team"
                class="mt-1 block w-full bg-black border-zinc-700 text-white focus:border-pl-green focus:ring-pl-green rounded-md shadow-sm">
                <option value="">Select a team...</option>
                @foreach(config('teams', []) as $team)
                    <option value="{{ $team }}" {{ old('favorite_team', $user->favorite_team) == $team ? 'selected' : '' }}>
                        {{ $team }}
                    </option>
                @endforeach
            </select>
